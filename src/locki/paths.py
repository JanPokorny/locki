...
if LEGACY.exists():
    ...
    STATE = pathlib.Path(os.environ.get("XDG_STATE_HOME") or (HOME / ".local" / "state")).expanduser().resolve() / "locki"
    RUNTIME = pathlib.Path(os.environ.get("XDG_RUNTIME_DIR") or os.environ.get("TMPDIR") or "/tmp").expanduser().resolve() / "locki"
