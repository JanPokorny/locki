...
2. Check current branch name using `git branch --show-current`. If it is `untitled#locki-<wt-id>`, reset it to the repo's default branch (detect with `git symbolic-ref --short refs/remotes/origin/HEAD` — yields e.g. `origin/main` or `origin/master`; fall back to whatever local branch HEAD tracked before the sandbox was created if there is no remote): `git reset --hard "$(git symbolic-ref --short refs/remotes/origin/HEAD 2>/dev/null || echo main)"`. Then rename using `git branch <new-name>#locki-<wt-id> --move`. Pick `<new-name>` based on the task at hand.
...
1. Check current branch name using `git branch --show-current`. If it is NOT `untitled#locki-<wt-id>`, run `git switch --force-create untitled#locki-<wt-id> "$(git symbolic-ref --short refs/remotes/origin/HEAD 2>/dev/null || echo origin/main)"` — this resolves to whatever the repo's default remote branch is (`origin/main`, `origin/master`, etc.) instead of assuming `main`.
