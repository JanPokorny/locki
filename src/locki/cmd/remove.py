...
def remove_cmd(match, interactive, force, branches, merged, as_json):
    ...
    if merged:
        ...
        targets = [
            ...
            if worktrees.is_merged(s.repo, trunk, s.branch)
            and (force or not s.path.exists() or not worktrees.has_uncommitted_changes(s, quiet=True))
        ]
    ...
    if worktree.path.exists() and not force and worktrees.has_uncommitted_changes(worktree):
        fail(
            f"Worktree for {worktree.branch} in {worktree.path} has uncommitted changes. Commit or stash them, or use --force."
        )
