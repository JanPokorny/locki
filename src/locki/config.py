...
USER_CONFIG_PATH = LOCKI_HOME / "config.toml"
...
def load_config(git_root: pathlib.Path) -> LockiConfig:
    """Load config from user (~/.locki/config.toml) and repo (locki.toml), repo takes precedence."""
    user_data: dict = {}
    if USER_CONFIG_PATH.exists():
        try:
            ...
            logger.warning("Invalid user config %s: %s", USER_CONFIG_PATH, e)
...
def save_user_config(section: str, key: str, value: object) -> None:
    ...
    data = tomlkit.loads(USER_CONFIG_PATH.read_text()) if USER_CONFIG_PATH.exists() else tomlkit.document()
    if section not in data:
        data.add(section, tomlkit.table())
    data[section][key] = value
    USER_CONFIG_PATH.write_text(tomlkit.dumps(data))
